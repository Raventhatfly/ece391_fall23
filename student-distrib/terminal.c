...
uint32_t buffer_clear(){
    for (i = 0; i < 128; i++) my_terminal.terminal_buffer[i] = ' '; /*clear the buffer of all the 128 chars*/
...
void terminal_write(){
    ...
    my_terminal.cursor_x_coord=screen_x;
    my_terminal.cursor_y_coord=screen_y;
    *(uint32_t *)(video_mem + ((COLS * screen_y + screen_x) * 2) + 1) = 0x7; /*set the ATTRIB of the screen, use 0x7 as it can't be directly link to lib.c*/
    draw_cursor(my_terminal.cursor_x_coord, my_terminal.cursor_y_coord); /*redraw the cursor*/
    my_terminal.buffer_iterator++;
    my_terminal.buffer_iterator= my_terminal.buffer_iterator % 128; /*if the buffer is full, set the iterator to 0*/
}
...
uint32_t terminal_clear(){
    ...
    for (i = 0; i < ROWS * COLS; i++) *(uint32_t *)(video_mem + i*2 + 1) = 0x7; /*set the ATTRIB of the screen, use 0x7 as it can't be directly link to lib.c*/
    i = 0;   
    screen_x = i;
    screen_y = i;
